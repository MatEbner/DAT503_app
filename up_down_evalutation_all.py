...
TEST_SPLIT_RATIO = 0.01                 # Aufteilung der Daten in Trainings- und Testdaten (z.b. 80% Training/20% Test)
...
###############################################################################
# 1. Feature-Engineering-Funktionen
###############################################################################
...
def time_based_train_test_split(df: pd.DataFrame, test_ratio: float):
    ...
    # TODO Splitten nach bestimmten Datum
...
def evaluate_model(
    ...
    # TODO: Benchmark: Buy&Hold je Tradefenster
    #buy_hold_curve = strat_df.groupby("Ticker").apply(
    #    lambda d: (d["Close"].shift(-horizon_days) / d["Close"] - 1).cumsum()
    #)
    ...
    plt.plot(equity_curve.index, equity_curve.values, label="Strategie-Kapitalverlauf")
...
def main():
    ...
    # Selektieren der wichtigsten Daten fürs Trainieren ohne Datum, Return_fwd und Target Spalte -> TODO: Warum nicht Return_fwd?
    ...
    # LightGBM Klassifier konfigurieren -> TODO: Was sind das für Parameters und wie kann man sie vielleicht optimieren?
    clf = LGBMClassifier(
        n_estimators=600,
        learning_rate=0.05,
        max_depth=6,
    ...
    for ticker_name, grp in model_df.groupby("Ticker"):
    ...
    latest_signals_df = pd.DataFrame(latest_signals).sort_values("ProbUp", ascending=False)
    # Ausgabe des Ergebnis - TODO: Last Date checken
    print("\n=== Aktuelle Einschätzung (pro Ticker, letztes Datum) ===")
